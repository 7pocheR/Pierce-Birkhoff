...
**Parameters:** `n=30`, `d=5`, `k=2`, using ordinary total degree and the
...
Each symmetric matrix has fifteen independent entries. The determinant has
degree exactly five, since its diagonal-product monomial has coefficient
one. Every label is a nonzero homogeneous quadratic, containing `XᵢᵢYᵢᵢ`
with coefficient one. Its sign tests and the pairwise label comparisons
have degree at most two; together with the determinant test they give the
degree-five partition. On `(X,Y)=(tI₅,tI₅)` for `t>0`, the function equals
`t²`, so the quadratic degree is attained by the function itself.
...
project are in [n=72_d=6_k=2](../n=72_d=6_k=2/).
