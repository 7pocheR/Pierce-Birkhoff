...
/-!
Independent executable checks for the eight-dimensional import closure.
These checks are audit tools and are not dependencies of the mathematical theorem.
-/

open Lean

namespace Quadratic8IndependentAudit
...
def expectedProjectModules : Array Name := #[
  ...
  `PBCounterexample.Quadratic8Data,
  `PBCounterexample.Quadratic8Geometry,
  `PBCounterexample.QuadraticSignPartition,
  `PBCounterexample.Quadratic8Partition,
  `PBCounterexample.Quadratic8Main]

def finalRoots : Array Name := #[
  ``PBCounterexample.Quadratic8.whole_space_counterexample,
  ``PBCounterexample.Quadratic8.exists_whole_space_counterexample,
  ``PBCounterexample.Quadratic8.not_isPolynomialLattice,
  ``PBCounterexample.Quadratic8.not_finite_sup_inf_polynomials,
  ``PBCounterexample.Quadratic8.f_graph,
  ``PBCounterexample.Quadratic8.minimum_eq,
  ``PBCounterexample.Quadratic8.orientation_ne_zero,
  ``PBCounterexample.Quadratic8.continuous_f,
  ``PBCounterexample.Quadratic8.signPartition_count,
  ``PBCounterexample.Quadratic8.agrees_on_entire_sign_realization,
  ``PBCounterexample.Quadratic8Geometry.determinant_bound,
...
def auditStage (message : String) : CoreM Unit :=
  Lean.Core.liftIOCore do
    ...
    out.putStrLn s!"QUADRATIC8_AUDIT_STAGE {message}"
    out.flush
...
def expectRejected {α : Type} (label fragment : String) (result : Except String α) :
  ...
  | .error reason =>
    ...
    logInfo m!"QUADRATIC8_NEGATIVE_CONTROL_OK {label}: {reason}"
...
def negativeControls (kenv : Kernel.Environment) : CoreM Unit := do
  let unknown : ConstantInfo := .axiomInfo {
    name := `Quadratic8AuditNegative.hiddenAxiom
    levelParams := []
    type := mkConst ``True
    isUnsafe := false }
  ...
  let hidden : ConstantInfo := .thmInfo {
    name := `Quadratic8AuditNegative.hiddenProofDependency
    levelParams := []
    type := mkConst ``True
  ...
  let missing : ConstantInfo := .thmInfo {
    name := `Quadratic8AuditNegative.missingProofDependency
    levelParams := []
    type := mkConst ``True
    value := mkConst `Quadratic8AuditNegative.doesNotExist }
  ...
  for safety in [DefinitionSafety.unsafe, DefinitionSafety.partial] do
    let bad : ConstantInfo := .defnInfo {
      name := `Quadratic8AuditNegative.operationalRoot
      levelParams := []
      type := mkConst ``True
  ...
  let expectedNames : Array Name := #[
    `Quadratic8AuditNegative.domain, `Quadratic8AuditNegative.letType,
    `Quadratic8AuditNegative.letValue, `Quadratic8AuditNegative.lambdaDomain,
    `Quadratic8AuditNegative.function, `Quadratic8AuditNegative.projectedType,
    `Quadratic8AuditNegative.argument]
  let expression :=
  ...
  logInfo "QUADRATIC8_NEGATIVE_CONTROL_OK every expression-bearing syntax position was traversed"
  let original := kenv.find? ``PBCounterexample.Quadratic8.whole_space_counterexample
  ...
  logInfo "QUADRATIC8_NEGATIVE_CONTROL_OK altered stored theorem body is detected by exact comparison"
  ...
  let selfReference : TheoremVal := {
    name := `Quadratic8AuditNegative.circularStoredVariant
    levelParams := []
    type := mkConst ``True
    value := mkConst `Quadratic8AuditNegative.circularStoredVariant }
  ...
  let bodyOnlyAxiom : TheoremVal := {
    selfReference with
    name := `Quadratic8AuditNegative.storedVariantHiddenAxiom
    value := mkConst unknown.name }
...
def operationalNegativeControls (env : Environment) : CoreM Unit := do
  ...
  for spec in operationalSpecs do
    ...
    rejectBoth "lookalike operational name" "unrecognized operational"
      { original with name := `Quadratic8AuditNegative.lookalike._unsafe_rec }
    ...
    let bridge : ConstantInfo := .defnInfo {
      name := `Quadratic8AuditNegative.operationalBridge, levelParams := [],
      type := mkConst ``Nat, value := mkConst spec.name [.zero],
      hints := .opaque, safety := .safe }
    let proofOnly : TheoremVal := {
      name := `Quadratic8AuditNegative.operationalProofBody, levelParams := [],
      type := mkConst ``True, value := mkConst spec.name [.zero] }
    let typeOnly : ConstantInfo := .thmInfo {
      name := `Quadratic8AuditNegative.operationalTypeOnly, levelParams := [],
      type := mkConst spec.name [.zero], value := mkConst ``True.intro }
    let indirect : ConstantInfo := .thmInfo {
      name := `Quadratic8AuditNegative.operationalIndirect, levelParams := [],
...
def enumerateProject (env : Environment) : CoreM ProjectInventory := do
  ...
  for i in [:env.header.moduleNames.size] do
    ...
    if (`PBCounterexample).isPrefixOf origin then
      ...
      logInfo m!"QUADRATIC8_PROJECT_MODULE {origin} records={data.constants.size}"
      for name in data.constNames, stored in data.constants do
        ...
        if operational then
          ...
          logInfo m!"QUADRATIC8_OPERATIONAL_STORED_RECORD_CHECKED {origin} {name} parent={spec.parent} complete_body_equal=true safety=partial hints=opaque all=[self] levels=[u_1] noncomputable={spec.noncomputableFlag}; not a mathematical root"
          logInfo m!"QUADRATIC8_OPERATIONAL_TYPE {name} {ci.type}"
          logInfo m!"QUADRATIC8_OPERATIONAL_BODY_DEPENDENCIES {name} {(rawDependencies ci).qsort Name.lt}"
        else
          ...
          | .ok (some info) =>
            ...
            logInfo m!"QUADRATIC8_STORED_PROOF_VARIANT_RETAINED stored={origin} name={name}; raw traversal and fresh-name kernel checking required"
        if idx.toNat != i then
          duplicates := duplicates + 1
          logInfo m!"QUADRATIC8_DUPLICATE_RECORD {name} stored={origin} first_registered={definingOrigin}"
        if names.contains name then continue
        names := names.insert name
        if !operational then
          roots := roots.push name
          logInfo m!"QUADRATIC8_PROJECT_DECL {definingOrigin} {declarationKind ci} {name}"
  for origin in expectedProjectModules do
    unless modules.contains origin do
      throwError "Expected eight-dimensional dependency module was omitted: {origin}"
    ...
    if let some idx := env.getModuleIdxFor? name then
      ...
      if (`PBCounterexample).isPrefixOf origin then
        ...
        if operational then
          ...
          logInfo m!"QUADRATIC8_OPERATIONAL_EFFECTIVE_RECORD_CHECKED {origin} {name}"
        if !names.contains name then
          names := names.insert name
          if !operational then roots := roots.push name
          logInfo m!"QUADRATIC8_EXTRA_PROJECT_DECL {origin} {declarationKind ci} {name} operational={operational}"
  ...
  logInfo m!"QUADRATIC8_PROJECT_INVENTORY_OK modules={modules.size} all_project_names={names.size} safe_roots={roots.size} records={records} duplicates={duplicates} retained_proof_variants={variants.size} stored_operational={storedOperational.size} effective_operational={effectiveOperational.size}"
  return {
    roots := roots
    variants := variants
...
def replayNegativeControl (kenv : Kernel.Environment) : CoreM Unit := do
  let badName := `Quadratic8AuditNegative.invalidProof
  ...
  let rejected : Bool ← Lean.Core.liftIOCore <| do
    ...
    try
      ...
      IO.println s!"QUADRATIC8_KERNEL_NEGATIVE_CONTROL_OK invalid proof rejected: {ex}"
      pure true
  unless rejected do
...
def storedVariantKernelNegativeControl (kenv : Kernel.Environment) : CoreM Unit := do
  let original : TheoremVal := {
    name := `Quadratic8AuditNegative.effectiveDuplicate
    levelParams := []
    type := mkConst ``True
    value := mkConst ``True.intro }
  ...
  let rejected : Bool ← Lean.Core.liftIOCore do
    ...
    try
      let _ ← kernelCheckStoredVariant checked retained
        `Quadratic8AuditNegative.freshInvalidDuplicate
      pure false
    catch ex =>
      unless (toString ex).contains "mismatch" do
        throw <| IO.userError s!"Alternate-body control failed for an unintended reason: {ex}"
      IO.println s!"QUADRATIC8_KERNEL_NEGATIVE_CONTROL_OK invalid alternate stored body rejected: {ex}"
      pure true
  unless rejected do
...
def runAudit (withReplay : Bool) : CoreM Unit := do
  ...
  for variant in inventory.variants do
    ...
    logInfo m!"QUADRATIC8_STORED_PROOF_VARIANT_RAW_OK stored={variant.origin} name={variant.info.name} constants={alternate.constants.size} axioms={alternate.axioms.toArray.qsort Name.lt}; kernel check pending"
  ...
  for name in inventory.operational do
    ...
    logInfo m!"QUADRATIC8_OPERATIONAL_UNREACHABLE_FROM_ALL_SAFE_ROOTS_AND_VARIANTS {name}"
  ...
  for name in roots do
    ...
    logInfo m!"QUADRATIC8_CACHED_AXIOMS {name} {reported}"
  ...
  for ax in allRawAxioms do
    ...
    logInfo m!"QUADRATIC8_STANDARD_AXIOM_SCHEMA_OK {ax} origin={actualOrigin} TYPE {ci.type}"
  logInfo m!"QUADRATIC8_INDEPENDENT_RAW_AUDIT_OK roots={roots.size} effective_constants={result.constants.size} combined_constants={replayConstants.size} definitions={result.definitions} theorems={result.theorems} stored_proof_variants={inventory.variants.size} axioms={allRawAxioms}"
  for name in finalRoots do
    let some ci := kenv.find? name | throwError "Missing final declaration {name}"
    logInfo m!"QUADRATIC8_FINAL_TYPE {name} {ci.type}"
  if withReplay then
    ...
    for variant in inventory.variants do
      let freshName := Name.mkNum `Quadratic8IndependentStoredProof.checked variantIndex
      ...
      logInfo m!"QUADRATIC8_STORED_PROOF_VARIANT_KERNEL_OK stored={variant.origin} original={variant.info.name} fresh_check={freshName}"
      variantIndex := variantIndex + 1
    auditStage "every retained stored proof body kernel-checked"
    logInfo m!"QUADRATIC8_INDEPENDENT_EMPTY_KERNEL_REPLAY_OK roots={roots.size} supplied_constants={replayConstants.size} stored_proof_variants={inventory.variants.size} axioms={allRawAxioms}"

end Quadratic8IndependentAudit

set_option maxHeartbeats 0 in
run_cmd Lean.Elab.Command.liftCoreM (Quadratic8IndependentAudit.runAudit true)
