...
**Parameters:** `n=6`, `d=3`, `k=2`, using ordinary total degree and the
...
- [Final formal statements](lean/PBCounterexample/Gram6Main.lean)
...
partition also uses the cubic polynomial `det A`. In the displayed coordinates,
`det A = −r(x²+y²)/2 + rxz/2 −xvz + yz(z+u)`, with nonzero cubic
coefficient `−1/2` on `rx²`. Each of the four labels is a nonzero homogeneous
quadratic. Their signs and pairwise comparison signs, together with the
determinant sign, give the degree-three partition. No claim of minimum
possible dimension, or of a resolution of PB(3), is made.

The standalone `lean` project contains only the 29 mathematical modules
...
[verification instructions](lean/README.md) for the precise scope and the
