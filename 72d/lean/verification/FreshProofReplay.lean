...
run_cmd do
  ...
  for name in seen do
    let some ci := kenv.find? name | throwError "Missing replay dependency {name}"
    replayConstants := replayConstants.insert name ci
  let empty ← mkEmptyEnvironment
  let checked ← empty.toKernelEnv.replay replayConstants
  let mut checkedRootCount : Nat := 0
  for name in roots do
    let some original := kenv.find? name | throwError "Missing original replay root {name}"
    let some verified := checked.find? name | throwError "Replay omitted mathematical root {name}"
    unless original.type == verified.type && original.levelParams == verified.levelParams do
      throwError "Replay changed the mathematical root statement {name}"
    checkedRootCount := checkedRootCount + 1
  for name in seen do
    unless (checked.find? name).isSome do
      throwError "Replay omitted a mathematical dependency {name}"
  logInfo m!"FRESH_DEPENDENCY_KERNEL_REPLAY_OK roots={checkedRootCount} supplied_constants={replayConstants.size} axioms={raw}"
