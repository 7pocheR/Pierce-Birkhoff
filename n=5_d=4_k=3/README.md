...
**Parameters:** `n=5`, `d=4`, `k=3`, using ordinary total degree and the
[repository convention](../README.md). The five coordinates are independent;
the derived sixth coordinate in the construction is a polynomial in them.
...
algebraic subset. The four nonzero labels have nonzero cubic terms. The
displayed partition also uses the determinant polynomial `D`, which has
degree four. Its homogeneous quartic part is
`−8a²(−ac − ce + 6c² + 2d² − 4bd + 2b²)`, so it does not vanish identically.
The labels and their pairwise differences have degree at most three;
adding their sign tests to that of `D` gives the displayed degree-four
partition. This does not assert that every partition for the same function
requires degree four. No claim of minimum possible dimension, or of a
resolution of PB(3) or PB(4), is made.
